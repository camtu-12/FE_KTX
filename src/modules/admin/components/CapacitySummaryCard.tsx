  ...
  return (
    <div className="rounded-2xl border border-[#d6e2f1] bg-[#f7faff] p-3">
      ...
      {capacity.total_beds <= 0 && (
        <div className="mt-3 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800">
          <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
          <span>Không tìm thấy giường thuộc phạm vi của đợt đăng ký.</span>
        </div>
      )}
      ...
      {capacity.capacity_exceeded ? (
        <div className="mt-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700">
          Số hồ sơ đang chọn duyệt vượt quá sức chứa hiện tại {valueOf(capacity.over_capacity_count)} suất. Có thể duyệt thêm: {valueOf(capacity.available_approval_slots)} hồ sơ, đang chọn duyệt: {valueOf(capacity.proposed_approved_count)} hồ sơ.
        </div>
        ...
        <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800">
          Hiện không còn suất để duyệt thêm.
        </div>
      ) : null}
